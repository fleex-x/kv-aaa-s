...
TEST_CASE("RAMMemoryManager simple") {
  ...
  ByteArrayPtr kvs = memory_manager.create_byte_array(MemoryPurpose::KVS);
  ByteArrayPtr skip_list_bl =
      memory_manager.create_byte_array(MemoryPurpose::SKIP_LIST_BL);
  ByteArrayPtr skip_list_ul =
      memory_manager.create_byte_array(MemoryPurpose::SKIP_LIST_UL);
  std::vector<ByteArrayPtr> sst;
  for (std::size_t i = 0; i < 20; ++i) {
    sst.push_back(memory_manager.create_byte_array(MemoryPurpose::SST, i));
  }

  CHECK(kvs == memory_manager.get_byte_array(MemoryPurpose::KVS));
  CHECK(skip_list_bl ==
        memory_manager.get_byte_array(MemoryPurpose::SKIP_LIST_BL));
  CHECK(skip_list_ul ==
        memory_manager.get_byte_array(MemoryPurpose::SKIP_LIST_UL));
  for (std::size_t i = 0; i < sst.size(); ++i) {
    CHECK(sst[i] == memory_manager.get_byte_array(MemoryPurpose::SST, i));
  }

  sst[2]->append(bytes({3, 4, 5}));
  CHECK(memory_manager.get_byte_array(MemoryPurpose::SST, 2)->read(0, 3) ==
        bytes({3, 4, 5}));
  ...
  CHECK(new_kvs == memory_manager.get_byte_array(MemoryPurpose::KVS));
...
TEST_CASE("RAMMemoryManager move-constructor") {
  ...
  ByteArrayPtr kvs = memory_manager1.create_byte_array(MemoryPurpose::KVS);
  ByteArrayPtr skip_list_bl =
      memory_manager1.create_byte_array(MemoryPurpose::SKIP_LIST_BL);
  ByteArrayPtr skip_list_ul =
      memory_manager1.create_byte_array(MemoryPurpose::SKIP_LIST_UL);
  std::vector<ByteArrayPtr> sst;
  for (std::size_t i = 0; i < 20; ++i) {
    sst.push_back(memory_manager1.create_byte_array(MemoryPurpose::SST, i));
  }
  ...
  /*test actions*/ {
    ...
    CHECK(new_sst_2 == memory_manager.get_byte_array(MemoryPurpose::SST, 2));
